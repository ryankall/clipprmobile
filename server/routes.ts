...
if (!stripe) {
  console.warn(
    "Stripe not configured. Set STRIPE_SECRET_KEY environment variable to enable payments.",
  );
}
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `📱 SMS Verification Code for ${user.phone}: ${verificationCode}`,
        );
  ...
  app.get("/api/booking-link", requireAuth, async (req, res) => {
    try {
      ...
      if (!user || !user.phone) {
        return res
          .status(400)
          .json({ message: "Phone number required to generate booking link" });
      }
      ...
      const businessSlug =
        user.businessName?.toLowerCase().replace(/\s+/g, "") || "clipcutman";
  ...
  app.get("/api/appointments", requireAuth, async (req, res) => {
    try {
      ...
      const appointments = await storage.getAppointmentsByUserId(
        userId,
        start,
        end,
      );
    ...
    try {
      ...
      if (!user.phoneVerified) {
        return res.status(403).json({
          ...
          details:
            "Your phone number must be verified before booking appointments. Please verify your phone in settings.",
        });
      }
      ...
      for (const serviceSelection of services) {
        ...
        if (!service) {
          return res.status(400).json({
            message: `Service with ID ${serviceSelection.serviceId} not found`,
          });
        }
      ...
      const appointmentEnd = new Date(
        appointmentStart.getTime() + totalCalendarDuration * 60 * 1000,
      );
      ...
      // Check for overlaps
      const overlappingAppointments = blockingAppointments.filter(
        (existingApt) => {
          const existingStart = new Date(existingApt.scheduledAt);
          const existingEnd = new Date(
            existingStart.getTime() + existingApt.duration * 60 * 1000,
          );
      ...
      if (overlappingAppointments.length > 0) {
        const conflictDetails = overlappingAppointments
          .map((apt) => {
        ...
        console.log(
          "BLOCKING APPOINTMENT CREATION due to overlap with:",
          conflictDetails,
        );
      ...
      console.log(
        "✅ NO OVERLAPS DETECTED - Proceeding with appointment creation",
      );
      ...
      console.log(
        `Creating ${serviceDetails.length} appointment services for appointment ${appointment.id}`,
      );
      for (const serviceDetail of serviceDetails) {
        try {
          console.log(
            `Creating appointment service: appointmentId=${appointment.id}, serviceId=${serviceDetail.service.id}, quantity=${serviceDetail.quantity}, price=${serviceDetail.price}`,
          );
          ...
          console.error(
            `❌ Failed to create appointment service:`,
            serviceError,
          );
  ...
  app.patch("/api/appointments/:id", requireAuth, async (req, res) => {
    try {
      ...
      if (
        ...
        console.log(
          "🔄 CONFIRMING PENDING APPOINTMENT - Running overlap check",
        );

        const appointmentStart = new Date(currentAppointment.scheduledAt);
        const appointmentEnd = new Date(
          appointmentStart.getTime() + currentAppointment.duration * 60 * 1000,
        );
        ...
        const confirmedAppointments = existingAppointments.filter(
          (apt) => apt.status === "confirmed" && apt.id !== appointmentId,
        );

        console.log(
          "Checking against confirmed appointments:",
        ...
        const overlappingAppointments = confirmedAppointments.filter(
          (existingApt) => {
            const existingStart = new Date(existingApt.scheduledAt);
            const existingEnd = new Date(
              existingStart.getTime() + existingApt.duration * 60 * 1000,
  ...
  app.delete("/api/appointments/:id", requireAuth, async (req, res) => {
    try {
      ...
      console.log(
        `Appointment ${appointmentId} deleted. SMS cancellation notice sent to ${appointment.client.name} at ${appointment.client.phone}`,
      );
  ...
  app.put("/api/clients/:id", requireAuth, async (req, res) => {
    try {
      ...
      console.log(
        `📝 Client update request - User ID: ${userId}, Client ID: ${clientId}`,
      );
  ...
  app.patch("/api/services/:id", requireAuth, async (req, res) => {
    try {
      ...
      if (serviceInUse) {
        return res.status(400).json({
          message:
            "Cannot edit service that is referenced in existing appointments. Please complete or cancel those appointments first.",
        });
      }
  ...
  app.delete("/api/services/:id", requireAuth, async (req, res) => {
    try {
      ...
      if (serviceInUse) {
        return res.status(400).json({
          message:
            "Cannot delete service that is referenced in existing appointments. Please complete or cancel those appointments first.",
        });
      }
      ...
      try {
        ...
        const photoData = insertGalleryPhotoSchema.parse({
          ...
          appointmentId: req.body.appointmentId
            ? parseInt(req.body.appointmentId)
            : null,
          fileSize: fileSize,
        });
      ...
      if (photo.fileSize) {
        ...
        if (user) {
          const newTotalSize = Math.max(
            0,
            (user.totalPhotoSize || 0) - photo.fileSize,
          );
          await storage.updateUser(userId, {
  ...
  app.post("/api/invoices", requireAuth, async (req, res) => {
    try {
      ...
      if (items && Array.isArray(items)) {
        ...
        console.log("No items to process or items is not an array:", {
          items,
        ...
        if (client) {
          ...
          if (sendEmail && client.email) {
            try {
              ...
              console.log(
                `Email notification would be sent to ${client.email} for invoice ${invoice.id}`,
              );
          ...
          if (sendSMS && client.phone) {
            try {
              ...
              console.log(
                `SMS notification would be sent to ${client.phone} for invoice ${invoice.id}`,
              );
  ...
  app.post("/api/invoices/:id/mark-paid", requireAuth, async (req, res) => {
    try {
      ...
      if (invoice.paymentStatus === "paid") {
        return res
          .status(400)
          .json({ error: "Invoice already marked as paid" });
      }
      ...
      if (invoice.paymentMethod !== "cash") {
        return res
          .status(400)
          .json({ error: "Only cash invoices can be marked as paid manually" });
      }
  ...
  app.post("/api/invoices/:id/undo-payment", requireAuth, async (req, res) => {
    try {
      ...
      if (invoice.paymentMethod !== "cash") {
        return res
          .status(400)
          .json({ error: "Only cash payments can be undone" });
      }
    ...
    try {
      ...
      if (emailSent) {
        ...
        res.status(500).json({
          success: false,
          message:
            "Failed to send export email. Please check email configuration.",
        });
      }
    } catch (error: any) {
    ...
    app.post("/api/stripe/request-refund", requireAuth, async (req, res) => {
      try {
        ...
        if (!user?.lastPaymentIntentId) {
          return res
            .status(400)
            .json({ message: "No payment found for refund" });
        }
        ...
        if (!subscriptionStart) {
          return res
            .status(400)
            .json({ message: "Subscription start date not found" });
        }
        ...
        if (diffInDays > 30) {
          return res.status(400).json({
            message:
              "Refund period has expired. Refunds are only available within 30 days of subscription start.",
          });
        }
        ...
        res
          .status(500)
          .json({ message: "Error processing refund: " + error.message });
      }
    });
      ...
      async (req, res) => {
        try {
          ...
          const isEligibleForRefund =
            ...
            (now.getTime() - user.subscriptionStartDate.getTime()) /
              (1000 * 60 * 60 * 24) <=
              30;
          ...
          res.json({
            ...
            refundDeadline: user.subscriptionStartDate
              ? new Date(
                  user.subscriptionStartDate.getTime() +
                    30 * 24 * 60 * 60 * 1000,
                )
              : null,
          });
          ...
          res.status(500).json({
            message: "Error getting subscription status: " + error.message,
          });
        }
      },
    );
      ...
      try {
        ...
        if (error.message.includes("signed up for Connect")) {
          res.status(400).json({
            ...
            details:
              "Please enable Stripe Connect in your Stripe dashboard first. Go to https://dashboard.stripe.com/connect/overview and complete the setup process.",
            setupRequired: true,
          });
        } else {
          res.status(500).json({
  ...
  app.post("/api/travel-time/calculate", requireAuth, async (req, res) => {
    try {
      ...
      if (clientAddress && appointmentTime) {
        ...
        const dayAppointments = await storage.getAppointmentsByUserId(
          userId,
          dayStart,
          dayEnd,
        );
        const confirmedAppointments = dayAppointments.filter(
          (apt) =>
            apt.status === "confirmed" &&
            new Date(apt.scheduledAt) < appointmentStart,
        );
        ...
        if (confirmedAppointments.length > 0) {
          const lastAppointment = confirmedAppointments.sort(
            (a, b) =>
  ...
  app.get(
    "/api/appointments/:date/travel-buffers",
    requireAuth,
    async (req, res) => {
      try {
        const userId = (req.user as any).id;
        ...
        const travelBuffers = await travelTimeService.calculateDayTravelBuffers(
          userId,
          appointmentDate,
        );
    ...
    async (req, res) => {
      try {
        ...
        if (!date || !clientAddress) {
          return res
            .status(400)
            .json({ message: "Date and client address are required" });
        }
  ...
  app.post(
    "/api/appointments/validate-scheduling",
    requireAuth,
    async (req, res) => {
      try {
        const userId = (req.user as any).id;
        ...
        if (!proposedStart || !proposedEnd || !clientAddress) {
          return res.status(400).json({
            message:
              "Proposed start time, end time, and client address are required",
          });
        }
        ...
        for (const appointment of existingAppointments) {
          ...
          const buffer = travelBuffers.find(
            (b) => b.appointmentId === appointment.id,
          );
          const bufferTime = buffer?.totalBuffer || 15;

          const bufferStart = new Date(
            appointmentStart.getTime() - bufferTime * 60 * 1000,
          );
          const bufferEnd = new Date(
            appointmentEnd.getTime() + bufferTime * 60 * 1000,
          );
  ...
  app.post(
    "/api/communications/send-message",
    requireAuth,
    async (req, res) => {
      try {
  ...
  app.post(
    "/api/communications/send-confirmation",
    requireAuth,
    async (req, res) => {
      try {
  ...
  app.get("/api/public/barber/:phone/next-available", async (req, res) => {
    try {
      ...
      for (const apt of appointments) {
        ...
        const bufferEnd = new Date(
          aptEnd.getTime() + postBufferMinutes * 60000,
        );
      ...
      res.json({
        ...
        conflictingAppointment: conflictingAppointment
          ? {
              ...
              endTime: new Date(
                new Date(conflictingAppointment.scheduledAt).getTime() +
                  conflictingAppointment.duration * 60000,
              )
                .toTimeString()
                .slice(0, 5),
        ...
        nextAvailableTime: nextAvailableTime
          ? nextAvailableTime.toTimeString().slice(0, 5)
          : null,
    ...
    try {
      ...
      for (let minutes = startMinutes; minutes < endMinutes; minutes += 15) {
        ...
        const slotDateTime = new Date(
          slotDateTimeLocal.getTime() + timezoneOffsetHours * 60 * 60 * 1000,
        );
        ...
        const isBooked = appointments.some((apt) => {
          ...
          const bufferStart = new Date(
            aptStart.getTime() - preBufferMinutes * 60000,
          );
          const bufferEnd = new Date(
            aptEnd.getTime() + postBufferMinutes * 60000,
          );
  ...
  app.post("/api/public/booking-request", async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message:
          "Booking request sent successfully. The barber will review and confirm your appointment.",
      });
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
  ...
  app.post("/api/reservations", requireAuth, async (req, res) => {
    try {
      ...
      if (services && services.length > 0) {
        requestedServices = userServices.filter((s) =>
  ...
  app.get("/api/reservations/pending", requireAuth, async (req, res) => {
    try {
      ...
      console.log(
        `Found ${pendingReservations.length} pending reservations:`,
        pendingReservations,
      );
  ...
  app.post("/api/anti-spam/process-booking", async (req, res) => {
    try {
      ...
      if (!barberId || !phoneNumber) {
        return res
          .status(400)
          .json({ message: "Barber ID and phone number are required" });
      }
      ...
      if (rateLimitEntry.length > 0) {
        ...
        if (now < resetTime) {
          ...
          if (entry.requestCount >= 3) {
            ...
            return res.status(429).json({
              message:
                "You've reached your daily limit for booking requests. Please try again tomorrow.",
              rateLimitInfo: {
                remainingRequests: 0,
                resetTime: resetTime,
              },
...
Travel required: ${message && message.includes("Travel: Yes") ? "Yes" : "No"}`;
  ...
  app.post("/api/reservations/:id/confirm", requireAuth, async (req, res) => {
    try {
      ...
      let client = await storage
        ...
        .then((clients) =>
          clients.find((c) => c.phone === reservation.customerPhone),
        );
  ...
  setInterval(
    async () => {
      try {
        ...
        const response = await fetch(
          "http://localhost:5000/api/reservations/expire",
          {
            method: "POST",
          },
        );
  ...
  app.get("/api/notifications/subscription", requireAuth, async (req, res) => {
    try {
      ...
      const subscription =
        await pushNotificationService.getSubscription(userId);
