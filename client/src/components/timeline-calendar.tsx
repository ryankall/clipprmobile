...
import { Clock, ChevronUp, ChevronDown, MapPin, Phone, DollarSign } from "lucide-react";
...
function generateTimeSlots(
  ...
  if (confirmedAppointments.length > 0) {
    ...
    // Expand range but preserve default bounds
    startHour = Math.min(startHour, earliestStart);
    endHour = Math.max(endHour, latestEnd);
  }
  ...
  for (let hour = startHour; hour <= endHour; hour++) {
    ...
    const hourAppointments = confirmedAppointments.filter(apt => {
      ...
      return startHour <= hour && (endHour > hour || (endHour === hour && endTime.getMinutes() > 0));
    });
...
function getServiceNamesDisplay(appointment: AppointmentWithRelations): string {
  ...
  return serviceName.length > 20 ? serviceName.slice(0, 20) + '...' : serviceName;
}

// Current time indicator
function getCurrentTimePosition(rowHeight: number = 80): { top: number; shouldShow: boolean } {
  const now = new Date();
...
export function TimelineCalendar({
  ...
  const appointmentPositions = calculateAppointmentPositions(dayAppointments, ROW_HEIGHT);
    ...
    <Card className="bg-dark-card border-steel/20 overflow-hidden">
      <CardContent className="p-0">
        ...
        {!collapsed && (
          <div 
            ref={containerRef}
            className="relative overflow-y-auto max-h-[600px] bg-white"
            ...
            <div className="relative">
              {timeSlots.map((slot, index) => (
                <div
                  ...
                  className={`
                    relative min-h-[80px] border-b border-gray-100 last:border-b-0
                    ${slot.isBlocked ? 'timeline-blocked-hours' : 'timeline-working-hours'}
                    ${!slot.isWithinWorkingHours ? 'opacity-50' : ''}
                  `}
                  style={{ height: `${ROW_HEIGHT}px` }}
                >
                  {/* Time label */}
                  <div className="absolute left-0 top-0 w-16 h-full flex items-start justify-center pt-2 bg-gray-50 border-r border-gray-200">
                  ...
                  {slot.isBlocked && (
                    <div className="absolute left-16 top-0 right-0 h-full flex items-center justify-center">
                      <span className="text-xs text-gray-400">Outside working hours</span>
                    </div>
                  )}
