...
```
Changelog:
- July 1, 2025. Modified appointment creation workflow to enforce reservation-first system - changed direct appointment creation to create reservations instead of appointments, all appointments now go through SMS confirmation workflow regardless of source (direct or public booking), updated appointment statuses to only include pending/confirmed/cancelled/no_show, removed scheduled status, ensured proper workflow where reservations convert to appointments only after SMS confirmation
