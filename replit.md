...
```
Changelog:
- July 3, 2025. TRAVEL TOGGLE IMPLEMENTATION & HOME SCREEN FIX - implemented complete travel yes/no toggle system that controls address field visibility and travel time inclusion in calendar blocking, added travel time storage in database with automatic calculation when client is selected, fixed home screen bug where deleted/cancelled appointments still showed in today's and next cards by filtering getTodayAppointments to only return confirmed/pending appointments, enhanced pending appointments to display stored travel time when present, created comprehensive unit test suite for travel toggle functionality (20 tests passing), travel time only included in overlap detection when travel toggle is enabled, address field shows/hides based on toggle state, travel time calculated once and saved as variable until modal closes
