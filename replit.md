...
```
Changelog:
- July 3, 2025. TRAVEL TIME DISPLAY & APPOINTMENT CREATION FIX - added travel time display to schedule appointment form showing service duration, travel time, and total duration with separate lines and icons, optimized travel time calculation to only run on prefill (not on every field change) to reduce API calls, fixed appointment creation issue where duplicate client phone numbers caused scheduling failures by implementing smart client lookup that uses existing clients when phone numbers match, enhanced duration card to show three distinct sections: service duration (scissors icon), travel time (car icon), and total duration (clock icon) with proper border separation
