...
```
Changelog:
- July 2, 2025. Fixed appointment booking workflow from messages to use proper redirect approach - updated "Book Appointment" button to redirect to appointment creation page with prefilled data instead of creating appointments directly, fixed URL path from /appointment-new to /appointments/new to match routing configuration, added comprehensive URL parameter handling for date, time, services, address, notes, client information, and custom services from booking messages, implemented notification banner when booking from message showing custom service details, form automatically prefills with all booking request details for barber review and approval before creating appointment
- July 2, 2025. Implemented automatic appointment expiration system - added missing auto-expiration logic for pending appointments that were created over 30 minutes ago, integrated expiration check into existing 5-minute background job alongside reservation expiration, expired appointments change status to "expired" and disappear from pending confirmations dashboard, system automatically creates barber notification messages when appointments expire, frees up time slots for new bookings, successfully tested with jake boo boo appointment (ID 49) which expired after 80 minutes and was properly processed by the system
